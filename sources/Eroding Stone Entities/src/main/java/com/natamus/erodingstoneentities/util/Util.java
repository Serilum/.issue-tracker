...
public class Util {
	...
	public static boolean populateArrays() {
		String tosandstring = ConfigHandler.GENERAL.itemsWhichErodeIntoSand.get();
		String toredsandstring = ConfigHandler.GENERAL.itemsWhichErodeIntoRedSand.get();
		String toclaystring = ConfigHandler.GENERAL.itemsWhichErodeIntoClay.get();
		...
		for (String itemstring : toclaystring.split(",")) {
			...
			Item item = ForgeRegistries.ITEMS.getValue(rl);
			if (ConfigHandler.GENERAL.erodeIntoClayBlockInsteadOfClayBall.get()) {
	...
	public static void transformItemEntity(World world, ItemEntity ie) {
		...
		ItemEntity newie = new ItemEntity(world, ie.getPosX(), ie.getPosY(), ie.getPosZ(), new ItemStack(toitem, stack.getCount()));
		ie.remove();
		world.addEntity(newie);
	}
