...
public class ExtendedTridentItem extends TridentItem {
	...
	public void onPlayerStoppedUsing(ItemStack stack, World worldIn, LivingEntity entityLiving, int timeLeft) {
		if (entityLiving instanceof PlayerEntity) {
			...
			if (i >= TranscendingTridentConfigHandler.GENERAL.tridentUseDuration.get()) {
				...
				if (j <= 0 || playerentity.isWet() || (PlayerFunctions.isHoldingWater(playerentity) || !TranscendingTridentConfigHandler.GENERAL.mustHoldBucketOfWater.get())) {
					if (!worldIn.isRemote) {
	...
	public ActionResult<ItemStack> onItemRightClick(World worldIn, PlayerEntity playerIn, Hand handIn) {
		...
		else if (EnchantmentHelper.getRiptideModifier(itemstack) > 0 && !playerIn.isWet() && (!PlayerFunctions.isHoldingWater(playerIn) && TranscendingTridentConfigHandler.GENERAL.mustHoldBucketOfWater.get())) {
			return new ActionResult<>(ActionResultType.FAIL, itemstack);
		} 
		else {
