...
public class CampfireSpawnandTweaksUtil {
			...
			if (filename.endsWith(".spawnpoint")) {
				...
				if (cs.length == 3) {
					if (NumberFunctions.isNumeric(cs[0]) && NumberFunctions.isNumeric(cs[1]) && NumberFunctions.isNumeric(cs[2])) {
						...
						CampfireSpawnandTweaksCampfireEvent.playercampfires.put(playername, new Pair<World, BlockPos>(world, spawnpos.immutable()));
					}
	...
	public static boolean setCampfireSpawn(World world, String playername, BlockPos campfirepos) {
		...
		CampfireSpawnandTweaksCampfireEvent.playercampfires.put(playername.toLowerCase(), new Pair<World, BlockPos>(world, campfirepos.immutable()));
		return true;
	}
