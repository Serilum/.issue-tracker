...
public class AreasGUIEvent extends IngameGui {
	...
	public AreasGUIEvent(Minecraft mcIn) {
		super(mcIn);
		mc = mcIn;
	}
	...
	public void renderOverlay(RenderGameOverlayEvent.Post e){
		...
		if (hudmessage != "") {
			...
			Color colour = new Color(AreasConfigHandler.HUD.HUD_RGB_R.get(), AreasConfigHandler.HUD.HUD_RGB_G.get(), AreasConfigHandler.HUD.HUD_RGB_B.get(), gopacity);
			if (rgb != "") {
			...
			fontRender.drawString(ms, hudmessage, (int)(Math.round((width / 2) / modifier) - stringWidth/2), AreasConfigHandler.HUD.HUDMessageHeightOffset.get(), colour.getRGB());
	...
	public void setHUDFade(String random) {
		...
		new Thread( new Runnable() {
	    	public void run()  {
	        	try  { Thread.sleep( AreasConfigHandler.HUD.HUDMessageFadeDelayMs.get() ); }
